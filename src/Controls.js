class Controls {
  constructor(game, engine) {
    ...
    this.pauseButton.addEventListener('click', this.clickPlayPause.bind(this));
  ...
  clickPlayPause(event) {
    ...
    event.target.innerHTML = (this.game.isPaused) ? 'Play' : 'Pause';
  }
  ...
  setSpeed(xPos, minX, maxX) {
    ...
    this.game.updateSpeed(speedRatio);
  }
  ...
  resetSlideHandlePos(event) {
    const speedRatio = this.game.speedRatio;
    const minX = this.sliderBar.offsetLeft - this.sliderHandle.clientWidth * .5;
